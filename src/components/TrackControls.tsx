...
export default function TrackControls({ track, selectedStepIndex, onChange, onTrackChange, bpm, onStepNoteChange, onStepNotesChange, onEffectsChange, onResetSettings, onClearNotes, onClearPattern, onResetTrack, onPreview, onRenderTrack, onComingSoon, playheadMs }: Props) {
  ...
  const isLoopLike = Boolean(track.assignedSample && (track.assignedSample.type === "loop" || track.assignedSample.isLong || track.loopMode !== "oneshot"));
  ...
  return (
    <div className="track-controls">
      ...
      {isLoopLike && <div className="loop-controls"><h4>Loop Controls</h4><div className="control-grid"><label>Loop Mode<select value={track.loopMode} onChange={(event) => onTrackChange(track.id, { loopMode: event.target.value as LoopMode })}><option value="play-full">Play Full</option><option value="cut-to-step-length">Cut to Step Length</option><option value="loop-region">Loop Region</option><option value="fit-to-steps-coming-soon" disabled>Fit to Steps (coming soon)</option></select></label><label>Loop Length Steps<select value={track.loopLengthSteps ?? 16} onChange={(event) => onTrackChange(track.id, { loopLengthSteps: Number(event.target.value) })}>{[1, 2, 4, 8, 16, 24, 32].map((count) => <option key={count} value={count}>{count}</option>)}</select></label></div><label className="toggle-row"><input type="checkbox" checked={track.retriggerLoop ?? false} onChange={(event) => onTrackChange(track.id, { retriggerLoop: event.target.checked })} /> Retrigger Loop (stops previous loop and restarts)</label><p className="hint">Sample length: {sampleSeconds ? `${sampleSeconds.toFixed(2)}s` : "Duration not loaded yet"}. Region length at current BPM: {regionSeconds.toFixed(2)}s.</p>{sampleSeconds && <p className="hint">{sampleSeconds > regionSeconds ? `Sample is ${(sampleSeconds - regionSeconds).toFixed(2)}s longer than region. Loop is longer than selected step region.` : `Region is ${(regionSeconds - sampleSeconds).toFixed(2)}s longer than sample. Loop is shorter than selected step region.`}</p>}</div>}
      ...
      <div className="nudge-row"><button type="button" onClick={() => onPreview(track)} disabled={!track.assignedSample}>Preview Track</button><button type="button" onClick={() => onRenderTrack(track, "processed")} disabled={!track.assignedSample}>Render to New Sample</button><button type="button" onClick={() => onRenderTrack(track, "dry")} disabled={!track.assignedSample}>Render to New Sample + Download WAV</button><button type="button" onClick={() => onComingSoon("Full FX render")} disabled={!track.assignedSample}>Full FX render</button><button type="button" onClick={() => onResetSettings(track.id)}>Reset Playback Settings</button><button type="button" onClick={() => onChange(track.id, { fadeInMs: 0, fadeOutMs: 5, fadeInCurve: "linear", fadeOutCurve: "linear" })}>Reset Fade</button><button type="button" onClick={() => onChange(track.id, { pitchSemitones: 0 })}>Reset Pitch</button><button type="button" onClick={() => onEffectsChange(track.id, [])}>Reset FX Params / Remove All FX</button><button type="button" onClick={() => onClearNotes(track.id)}>Clear Chord/Notes on Selected Step</button><button type="button" onClick={() => onClearPattern(track.id)}>Clear Pattern</button><button type="button" onClick={() => onResetTrack(track.id)}>Reset Track</button></div>
