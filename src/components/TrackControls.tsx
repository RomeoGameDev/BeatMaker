...
export default function TrackControls({ track, selectedStepIndex, onChange, onTrackChange, bpm, onStepNoteChange, onStepNotesChange, onEffectsChange, onResetSettings, onClearNotes, onClearPattern, onResetTrack, onPreview, onRenderTrack, onComingSoon, playheadMs }: Props) {
  ...
  return (
    <div className="track-controls">
      ...
      <div className="waveform-toolbar"><span>Waveform</span>{(["original", "processed", "overlay"] as WaveformMode[]).map((mode) => <button type="button" key={mode} className={waveformMode === mode ? "active-filter" : ""} onClick={() => setWaveformMode(mode)}>{mode[0].toUpperCase() + mode.slice(1)}</button>)}</div><SampleWaveform sample={track.assignedSample} settings={track.settings} playheadMs={playheadMs} processed={waveformMode !== "original"} mode={waveformMode} /><p className="hint">Processed waveform approximates selected region, fade, pitch length, and volume. FX waveform render coming soon.</p>
      <section className="region-controls" aria-label="Sample region selection"><div className="region-readout"><strong>Region</strong><span>Start: {formatMs(regionStart)}</span><span>End: {formatMs(regionEnd)}</span><span>Length: {formatMs(regionLength)}</span></div><label className="slider-control"><span className="slider-label"><strong>Region Start</strong><em>{formatMs(regionStart)}</em></span><input type="range" min={0} max={sampleMs - 1} step={10} value={regionStart} onChange={(event) => setRegionStart(Number(event.target.value))} /><input type="number" min={0} max={sampleMs - 1} step={10} value={regionStart} onChange={(event) => setRegionStart(Number(event.target.value))} /></label><label className="slider-control"><span className="slider-label"><strong>Region End</strong><em>{formatMs(regionEnd)}</em></span><input type="range" min={1} max={sampleMs} step={10} value={regionEnd} onChange={(event) => setRegionEnd(Number(event.target.value))} /><input type="number" min={1} max={sampleMs} step={10} value={regionEnd} onChange={(event) => setRegionEnd(Number(event.target.value))} /></label><div className="nudge-row"><button type="button" onClick={() => setRegionStart(regionStart - 10)}>Start -10ms</button><button type="button" onClick={() => setRegionStart(regionStart + 10)}>Start +10ms</button><button type="button" onClick={() => setRegionEnd(regionEnd - 10)}>End -10ms</button><button type="button" onClick={() => setRegionEnd(regionEnd + 10)}>End +10ms</button><button type="button" onClick={() => setRegion(0, sampleMs)}>Set Region to Full Sample</button><button type="button" onClick={() => setQuickSteps(1)}>Quick 1 Step</button><button type="button" onClick={() => setQuickSteps(2)}>Quick 2 Steps</button><button type="button" onClick={() => setQuickSteps(4)}>Quick 4 Steps</button></div></section>
