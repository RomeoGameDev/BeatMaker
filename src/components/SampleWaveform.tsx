...
export default function SampleWaveform({ sample, settings, playheadMs, processed = false, mode = "original", slices = [], selectedSliceId, selection, onSelectionChange, onBoundaryChange, interactive = false }: Props) {
  ...
  function updateDrag(event: PointerEvent<HTMLElement>) { const drag = dragRef.current; if (!drag || !interactive) return; const ms = eventMs(event); if (drag.kind === "selection") onSelectionChange?.({ startMs: Math.min(drag.anchorMs, ms), endMs: Math.max(drag.anchorMs, ms) }); else onBoundaryChange?.({ sliceId: drag.sliceId, edge: drag.edge }, ms); }
  ...
  function finishDrag(event: PointerEvent<HTMLElement>) { updateDrag(event); dragRef.current = null; }
