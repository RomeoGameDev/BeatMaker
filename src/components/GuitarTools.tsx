...
export default function GuitarTools({ samples, bpm, track, selectedStepIndex, onStepNotesChange, onAddRenderedSample, onStatus }: Props) {
  ...
  function generateRiffTab() {
    if (!selectedFrets.length) { setTab("Select notes on the fretboard to generate exact tab."); onStatus("Select notes on the fretboard to generate exact tab."); return; }
    const cells = emptyTab(Math.max(12, selectedFrets.length * 4)).map((line) => line.split(""));
    selectedFrets.forEach((item, index) => {
      const row = 5 - item.stringIndex;
      const col = 3 + index * 4;
      String(item.fret).split("").forEach((char, offset) => { cells[row][col + offset] = char; });
    });
    setTab(cells.map((chars) => chars.join("")).join("\n"));
    onStatus("Generated tab from riff fretboard notes.");
  }
  async function copyTab() { try { await navigator.clipboard?.writeText(tab); onStatus("Generated tab copied to clipboard."); } catch { onStatus("Could not copy tab to clipboard."); } }
  return <div className="guitar-tools"><div className="control-grid compact-grid"><label>Mode<select value={mode} onChange={(e) => setMode(e.target.value as GuitarLabMode)}><option value="chord">Chord</option><option value="riff">Riff</option></select></label>{mode === "riff" && <label>Note Duration<select value={noteDuration} onChange={(e) => setNoteDuration(e.target.value as NoteDuration)}><option>1/4</option><option>1/8</option><option>1/16</option></select></label>}</div><p className="hint">{mode === "chord" ? "Chord: selected notes play together." : "Riff: selected notes play in order."}</p>
    ...
    <Section title="Generated Tab" defaultOpen={false}><p className="hint">Generated ASCII tab from Guitar Tools selections. Click fretboard cells for exact string/fret output; piano-only notes need fretboard positions first.</p><textarea className="tab-scratchpad" value={tab} readOnly placeholder="Generate tab from selected fretboard notes or a riff." rows={6} spellCheck={false} /><div className="nudge-row"><button onClick={generateChordTab}>Generate Tab from Selected Notes</button><button onClick={generateRiffTab}>Generate Tab from Riff</button><button onClick={() => void copyTab()} disabled={!tab}>Copy Tab to Clipboard</button><button onClick={() => { setTab(""); onStatus("Generated tab cleared."); }}>Clear Generated Tab</button></div></Section>
  </div>;
}
