...
type Props = { samples: Sample[]; bpm: number; track?: SequencerTrack; selectedStepIndex?: number; onStepNotesChange: (trackId: number, stepIndex: number, notes: string[]) => void; onAddRenderedSample: (input: { blob: Blob; id: string; name: string; filename: string; type: Sample["type"]; category: Sample["category"]; durationSeconds?: number; metadata?: Record<string, unknown> }) => Promise<Sample>; onStatus: (message: string) => void; };
...
export default function GuitarTools({ samples, bpm, track, selectedStepIndex, onStepNotesChange, onAddRenderedSample, onStatus }: Props) {
  ...
  async function preview(notes = playableNotes) { if (!source) { onStatus("Select a source sample to preview or render chords."); return; } const result = await previewPitchedNotes({ sample: source, rootNote, notes, mode: "chord", bpm, noteDuration, effects: fx }); if (result.status === "decode-failed") { await playHtmlAudioFallback(source); onStatus(sourceWarning ?? result.message); } else onStatus(result.message); }
  ...
  function send(notes = playableNotes) { if (!track || selectedStepIndex === undefined || track.mode !== "keyboard") { onStatus("Switch selected track to Keyboard mode to send notes."); return; } onStepNotesChange(track.id, selectedStepIndex, notes); onStatus("Sent selected chord/notes to the selected step."); }
