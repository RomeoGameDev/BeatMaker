    ...
    <div className="sequencer-panel">
      ...
      {tracks.map((track) => {
        ...
        <div style={trackStyle} className={`track-row ${selectedTrackId === track.id ? "selected-track" : ""} ${track.mode === "sliced" ? "sliced-track" : ""}`} key={track.id} onClick={() => onSelectTrack(track.id)}>
          ...
          <div className="track-right-controls" onClick={(event) => event.stopPropagation()}><label className="quick-volume"><span>Vol</span><input type="range" min={0} max={1.5} step={0.01} value={track.settings.volume} onChange={(event) => onTrackSettingsChange(track.id, { volume: Number(event.target.value) })} /><em>{Math.round(track.settings.volume * 100)}%</em></label><label className="quick-volume"><span>Pitch</span><input type="range" min={-24} max={24} step={1} value={track.settings.pitchSemitones} onChange={(event) => onTrackSettingsChange(track.id, { pitchSemitones: Number(event.target.value) })} /><em>{track.settings.pitchSemitones} st</em></label><div className="track-header-buttons"><button type="button" className={track.settings.mute ? "mute-button active" : "mute-button"} onClick={() => onToggleMute(track.id)}>M</button><button type="button" className={track.settings.solo ? "solo-button active" : "solo-button"} onClick={() => onToggleSolo(track.id)}>S</button>{tracks.length > 1 && <button type="button" className="remove-track" onClick={() => onRemoveTrack(track.id)}>×</button>}</div></div>
        </div>
      );})}
    </div>
  );
