    ...
    <div className="sequencer-panel">
      ...
      {tracks.map((track) => {
        ...
        <div style={trackStyle} className={`track-row ${selectedTrackId === track.id ? "selected-track" : ""} ${track.mode === "sliced" ? "sliced-track" : ""}`} key={track.id} onClick={() => onSelectTrack(track.id)}>
          ...
          {track.mode === "sliced" ? <div className="slice-step-rows">{(track.slices ?? []).map((slice) => <div className="slice-step-row" key={slice.id}><div className="slice-step-label"><span className="slice-dot" style={{ background: slice.color }} />{slice.name}</div><div className="steps" style={{ gridTemplateColumns: `repeat(${stepCount}, 1fr)` }}>{(track.sliceSteps?.[slice.id] ?? []).slice(0, stepCount).map((step, stepIndex) => <button key={stepIndex} className={`step ${step.active ? "on" : ""} ${currentStep === stepIndex ? "playing" : ""}`} style={step.active ? { backgroundColor: slice.color ?? trackColor } : undefined} onClick={(event) => { event.stopPropagation(); onToggleSliceStep(track.id, slice.id, stepIndex); }} aria-label={`${track.name} ${slice.name} step ${stepIndex + 1}`} />)}</div></div>)}</div> : <div className="steps" style={{ gridTemplateColumns: `repeat(${stepCount}, 1fr)` }}>
          ...
          <div className="track-right-controls mixer-strip" onClick={(event) => event.stopPropagation()}>
            <label className="quick-volume mixer-volume"><span>Vol</span><input type="range" min={0} max={1.5} step={0.01} value={track.settings.volume} onChange={(event) => onTrackSettingsChange(track.id, { volume: Number(event.target.value) })} /><em>{Math.round(track.settings.volume * 100)}%</em></label>
            <div className="pitch-control" aria-label={`${track.name} pitch controls`}><span>Pitch</span><div className="pitch-buttons">{[-12, -1, 0, 1, 12].map((amount) => <button type="button" key={amount} onClick={() => onTrackSettingsChange(track.id, { pitchSemitones: amount === 0 ? 0 : Math.max(-24, Math.min(24, track.settings.pitchSemitones + amount)) })}>{amount > 0 ? `+${amount}` : amount}</button>)}</div><em>{track.settings.pitchSemitones} st</em></div>
            <div className="track-header-buttons"><button type="button" className={track.settings.mute ? "mute-button active" : "mute-button"} onClick={() => onToggleMute(track.id)} aria-pressed={track.settings.mute}>M</button><button type="button" className={track.settings.solo ? "solo-button active" : "solo-button"} onClick={() => onToggleSolo(track.id)} aria-pressed={track.settings.solo}>S</button>{tracks.length > 1 && <button type="button" className="remove-track" onClick={() => onRemoveTrack(track.id)} aria-label={`Remove ${track.name}`}>×</button>}</div>
          </div>
        </div>
      );})}
    </div>
